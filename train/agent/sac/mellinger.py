...
class DifferentiableMellinger(nn.Module):
    ...
    def __init__(self, max_rpm=21600, ctrl_freq : int = 240, output = "pwm"):
        ...
        self.kd_z = torch.nn.Parameter(torch.tensor(0.4, requires_grad=True))
        # Attitude
        self.kR_xy = torch.nn.Parameter(torch.tensor(70000., requires_grad=True))
        self.kw_xy = torch.nn.Parameter(torch.tensor(0., requires_grad=True))
        self.ki_m_xy = torch.nn.Parameter(torch.tensor(20000., requires_grad=True))

        self.kR_z = torch.nn.Parameter(torch.tensor(60000., requires_grad=True))
        self.kw_z = torch.nn.Parameter(torch.tensor(500., requires_grad=True))
        self.ki_m_z = torch.nn.Parameter(torch.tensor(12000., requires_grad=True))

        self.P_COEFF_FOR = torch.stack([self.kp_xy, self.kp_xy, self.kp_z])
        self.I_COEFF_FOR = torch.stack([self.ki_xy, self.ki_xy, self.ki_z])
        self.D_COEFF_FOR = torch.stack([self.kd_xy, self.kd_xy, self.kd_z])
        self.P_COEFF_TOR = torch.stack([self.kR_xy, self.kR_xy, self.kR_z])
        self.I_COEFF_TOR = torch.stack([self.kw_xy, self.kw_xy, self.kw_z])
        self.D_COEFF_TOR = torch.stack([self.ki_m_xy, self.ki_m_xy, self.ki_m_z])
    ...
    def reset(self):
        ...
        self.last_pos_e = torch.zeros(3)
        self.integral_pos_e = torch.zeros(3)
    ...
    def forward(self, obs):
        ...
        # position control
        pos_e = torch.tensor([0., 0., 1.]) - obs[0:3]
        cur_quat = obs[3:7]
        cur_rpy = obs[7:10]
        cur_vel = obs[10:13]
        intergral_pos_error = obs[16:19]
        ...
        target_thrust = torch.multiply(self.P_COEFF_FOR, pos_e) \
                        + torch.multiply(self.I_COEFF_FOR, self.integral_pos_e) \
                        + torch.multiply(self.D_COEFF_FOR, vel_e) + torch.tensor([0, 0, self.GRAVITY]) # , device=self.de
        scalar_thrust = torch.clamp(torch.inner(target_thrust, cur_rotation[:, 2]), 0, torch.inf)
        ...
        target_z_ax = target_thrust / torch.norm(target_thrust)
        target_x_c = torch.tensor([1., 0., 0.]) # assume target rpy always 0
        target_y_ax = torch.cross(target_z_ax, target_x_c) / torch.norm(torch.cross(target_z_ax, target_x_c))
        target_x_ax = torch.cross(target_y_ax, target_z_ax)
        target_rotation = torch.vstack([target_x_ax, target_y_ax, target_z_ax]).T
        #### Target rotation #######################################
        ...
        rot_matrix_e = torch.matmul((target_rotation.T), cur_rotation) - torch.matmul(cur_rotation.T, target_rotation)
        rot_e = torch.stack([rot_matrix_e[2, 1], rot_matrix_e[0, 2], rot_matrix_e[1, 0]])
        ...
        pwm = thrust_pwm + torch.matmul(self.MIXER_MATRIX, target_torques)
...
def test_mellinger_controller():
    ...
    for i in range(500):
        ...
        xyz.append(env.intergral_rpy_e)
